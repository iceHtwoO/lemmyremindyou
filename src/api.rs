...
pub async fn comment(auth: &str, content: &str, parent_id: i32, post_id: i32) -> Result<(), Error>{
    ...
    client.post(url_builder("api/v3/comment", "").await)
    .json(&body)
    .send()
    .await
...
pub async fn mark_read(auth: &str, person_mention_id: i32) -> Result<(), Error>{
    ...
    let client = reqwest::Client::new();
    client.post(url_builder("api/v3/user/mention/mark_as_read", "").await)
    .json(&body)
    .send()
    .await?;
    debug!("Read Mention");
...
pub async fn get_auth_token() -> Result<String, Box<dyn std::error::Error>>{
    let body = json!({
        "username_or_email": std::env::var("USERNAME").expect("USERNAME not Set"),
        "password": std::env::var("PASSWORD").expect("PASSWORD not Set")
    });

    let client = reqwest::Client::new();
    let out = client.post(url_builder("api/v3/user/login", "").await)
    .json(&body)
    .send()
    .await?
